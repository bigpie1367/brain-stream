---
name: devops
description: Docker 빌드·환경설정·의존성·인프라 작업이 필요할 때 사용. Dockerfile, docker-compose.yml, requirements.txt, beets/config.yaml, config.yaml.example 변경 작업에 자동 호출됨. 패키지 추가, 빌드 오류, 환경변수, 볼륨 마운트 관련 작업 포함. (config.yaml.example 삭제됨 — 설정은 환경변수 전용)
...
## 작업 원칙
- 작업 전 관련 파일 먼저 읽기
- `beets/config.yaml` 변경: Dockerfile에 번들 → 재빌드 후 push 필요
- `Dockerfile`, `requirements.txt` 변경: 반드시 재빌드 후 push 필요
