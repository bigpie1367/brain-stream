...
1. `http://localhost:4533` 접속 (LAN에서만 접근 가능)
...
### 4.2 방화벽 설정 (navidrome 외부 차단)

navidrome(:4533)은 LAN 초기 설정 접근용으로만 포트를 열어둡니다.
외부에서 직접 접근하지 못하도록 방화벽에서 차단합니다.

```bash
# UFW 예시: 4533 포트를 LAN(192.168.1.0/24) 대역에서만 허용
sudo ufw allow from 192.168.1.0/24 to any port 4533
sudo ufw deny 4533

# 외부에서의 Subsonic API 접근은 brainstream(:8080) /rest/* 프록시를 통해 이루어짐
# stream.example.com/rest/ping → brainstream → navidrome:4533
```
