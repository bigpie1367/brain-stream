...
**중복 감지**: 동일 artist + track 조합의 `done` / `downloading` / `pending` 레코드가 이미 존재하면 새 잡을 생성하지 않고 기존 레코드를 `{"duplicate": true, "existing": {...}}` 형태로 반환한다 (HTTP 200).
...
| search | string | N | — | 아티스트명/트랙명/앨범명 부분 일치 검색 (max 500자) |
