...
|--------|------|------|
| GET | `/` | Web UI |
| GET | `/health` | Liveness probe (헬스체크) |
...
| DELETE | `/api/downloads` | 트랙 일괄 삭제 (파일 제거 + state.db를 ignored로 마킹) |
...
| GET | `/api/settings/pipeline-interval` | 현재 파이프라인 실행 주기 조회 |
| PUT | `/api/settings/pipeline-interval` | 파이프라인 실행 주기 변경 |
...
컨테이너 헬스체크용 liveness probe 엔드포인트.
...
인증 불필요. Docker HEALTHCHECK 또는 로드밸런서 헬스체크에 사용.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| artist | string | Y | 아티스트명 (max 500자) |
| track | string | Y | 트랙명 (max 500자) |
...
| Status | 설명 |
|--------|------|
| 422 | 입력값 검증 실패 (max_length 초과 등) |
| 409 | 동일 artist+track의 done/downloading/pending 레코드가 이미 존재 |
| 429 | Rate limit 초과 (10 req/min). `Retry-After` 헤더 포함 |
| 503 | 서버 설정 미로드 (기동 직후 일시적) |

**중복 감지**: 동일 artist + track 조합의 `done` / `downloading` / `pending` 레코드가 이미 존재하면 새 잡을 생성하지 않고 `409 Conflict`를 반환한다. 응답 body: `{"detail": "이미 존재: {mbid} ({status})"}`.
...
다운로드 이력을 페이지네이션으로 조회한다. `ignored` 상태 레코드는 항상 제외된다.

**Query Parameters**

| 파라미터 | 타입 | 필수 | 기본값 | 설명 |
|---------|------|------|--------|------|
| limit | integer | N | 100 | 한 페이지 최대 항목 수 (1~500) |
| offset | integer | N | 0 | 건너뛸 항목 수 |
| search | string | N | — | 아티스트명/트랙명/앨범명 부분 일치 검색 (max 200자) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| items | array | 다운로드 레코드 목록 |
| total | integer | 검색 조건에 맞는 전체 레코드 수 (페이지네이션 전) |
| limit | integer | 요청된 limit 값 |
| offset | integer | 요청된 offset 값 |
...
| status | string | `pending` / `queued` / `downloading` / `done` / `failed` |
...
라이브러리에서 여러 트랙을 일괄 삭제한다. 물리적 파일을 제거하고 state.db 레코드를 `ignored`로 마킹한 후 Navidrome 재스캔을 트리거한다.
...
```json
{
  "mbids": ["a3e48b38-...", "manual-a1b2c3d4"]
}
```

| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| mbids | string[] | Y | 삭제할 mbid 목록 (최소 1개) |
...
| 필드 | 타입 | 설명 |
|------|------|------|
| deleted | integer | `ignored`로 마킹된 레코드 수 |
| files_removed | integer | 실제 삭제된 파일 수 |
| errors | array | 개별 mbid 삭제 실패 정보 (`[{"mbid": "...", "error": "..."}]`) |
...
4. 전체 완료 후 Navidrome 재스캔 트리거 (파일이 삭제된 경우)
...
| 필드 | 타입 | 설명 |
|------|------|------|
| interval_hours | integer | 현재 파이프라인 실행 주기 (시간 단위). state.db `settings` 테이블에서 읽음. 미설정 시 기본값 6 |

---

## PUT `/api/settings/pipeline-interval`

파이프라인 실행 주기를 변경한다. 변경된 값은 state.db `settings` 테이블에 즉시 저장되며, 다음 스케줄러 틱부터 반영된다.
...
| 필드 | 타입 | 필수 | 설명 |
|------|------|------|------|
| interval_hours | integer | Y | 파이프라인 실행 주기 (시간 단위, 1~24) |
...
done → DELETE /api/downloads → ignored (파일 삭제, DB 레코드 유지)
```
...
인메모리 슬라이딩 윈도우 방식으로 POST/DELETE 엔드포인트에 Rate Limiting을 적용한다.
...
| `PUT /api/settings/pipeline-interval` | 10 req/min |
| GET 엔드포인트 | 제한 없음 |

- **키**: 클라이언트 IP (`request.client.host`) + 엔드포인트 패턴
- **윈도우**: 60초 슬라이딩 윈도우
- **초과 시**: `429 Too Many Requests` + `Retry-After: 60` 헤더
- **저장**: 인메모리 (서버 재시작 시 리셋)

## 입력값 검증

모든 문자열 입력에 최대 길이 제한을 적용한다.

| 대상 | 제한 |
|------|------|
| POST body 문자열 필드 (artist, track, album 등) | max 500자 |
| POST body ID 필드 (song_id, mbid, mb_recording_id 등) | max 100자 |
| POST body URL 필드 (cover_url) | max 2000자 |
| GET query params (artist, track) | max 500자 |

초과 시 `422 Unprocessable Entity` 반환 (Pydantic 자동 검증).

---

## 프록시 엔드포인트

brainstream은 Navidrome을 Docker 내부에서만 접근 가능하게 유지하면서 외부 클라이언트에 필요한 프록시 경로를 제공한다.

| Method | Path | 설명 |
|--------|------|------|
| GET, POST, HEAD | `/rest/{path}` | Subsonic API 프록시 — 외부 클라이언트(Amperfy 등)의 요청을 navidrome:4533으로 중계 |
| GET | `/api/subsonic/{path}` | 인증 자동 주입 Subsonic API 프록시 — MD5 토큰을 자동 생성하여 Navidrome에 전달 |
| GET | `/navidrome` | `/navidrome/`로 리다이렉트 |
| ALL | `/navidrome/{path}` | Navidrome 웹 UI 투명 프록시 |

이 엔드포인트들은 Rate Limiting 대상이 아니다.
