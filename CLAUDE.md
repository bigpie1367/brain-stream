...
- **Worker thread** (non-daemon): `worker_loop()` — FIFO 큐에서 잡을 꺼내 하나씩 순차 처리. `_shutdown_event` 기반 graceful shutdown (atexit 핸들러, join timeout=30s)
...
- **Graceful shutdown**: `atexit` 핸들러가 `_shutdown_event.set()` → 워커 스레드 join(30s). Docker `stop_grace_period: 40s`
...
- **SSE 큐 TTL**: `_job_queues`에 last-activity 타임스탬프 저장, 30분 비활성 시 자동 정리
...
| `src/worker.py` | Shared work queue module; `_work_queue` (FIFO), `_job_queues` (SSE, last-activity TTL), `_shutdown_event`, `enqueue_job()`, `emit()`, `worker_loop()`, `_cleanup_expired_queues()` |
