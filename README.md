...
컨테이너 시작 시 즉시 1회, 이후 **6시간마다** 자동으로 실행됩니다.

```
ListenBrainz CF 추천 목록 수신
  → MusicBrainz에서 아티스트·트랙 메타데이터 조회 (4단계 폴백 검색)
  → YouTube에서 오디오 검색 및 다운로드 (FLAC 우선, Opus fallback)
     라이브·커버 영상 자동 필터링, 차단 영상 감지 시 다음 후보로 retry
...
전체 다운로드 이력(최신 100건)을 테이블로 확인할 수 있습니다. 아티스트, 트랙명, 앨범명, 출처(ListenBrainz / Manual), 상태, MusicBrainz 링크, 다운로드 시각이 표시됩니다.

완료된 트랙은 ▶ 버튼으로 브라우저에서 바로 재생할 수 있습니다.
